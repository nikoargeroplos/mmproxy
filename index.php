...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ Credits
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
// Maintained for 1MB by Anthony Rossbach, Slightly edited for use with minme.
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
if (isset($_GET["rules"])){
	//Custom rules selector for minme panel
...
if (!file_exists("proxy_username.ruleconf")){
	...
	$username = !empty($_GET["username"]) ? strtolower($_GET["username"]) : false;
	if ($username === false) {
		echo "<html><head></head><body><h1 style='text-align:center;margin-top:150px;'>This custom domain is not setup, click the link on the minme dashboard to finish setup.</h1></body></html>";
		exit();
	} else {
		file_put_contents("proxy_username.ruleconf", $username);
		echo "<h1>Success! This domain will now proxy traffic for https://$username.minme.site</h1>";
...
$httpprefix="http://";
$source=''.$username.'.minme.site';
$usecache=false;
$url = 'https://'.$username.'.minme.site' . $link_request;
...
if (file_exists("proxy_rules_cache.ruleconf")){
	...
	if ($ruleis=="false"){
		$usecache=false;
		$allowcache=false;
		header("minmeproxy-cache-time: disabled");
	}
}
...
if (file_exists("cache_static_".$cachecode."_timestamp.txt")){
	if ($allowcache==true){
		...
		if ($checktime<=$cachetime){
			...
			header("minmeproxy-cache-time: ".$timeleft."");
			header("minmeproxy-cache-archive: ".$cachetime."");
			header("minmeproxy-cache-now: ".$checktime."");
		}else{
			//Nope cache expired
			$usecache=false;
			header("minmeproxy-cache-time: refresh");
		}
	}
}
...
if ($usecache==false){
	...
	if (curl_error($ch)){
		...
		header("Content-type: $type");
		if (!file_exists("cache_realtime_".$cachecode."_content.txt")){
			header("minmeproxy-cache: saved");
		}else{
			header("minmeproxy-cache: updated");
		}
